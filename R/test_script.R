...
test_hydro <- hydroGraphR::dl_hydro(c("08HB086", "08HC004"))


st_df_past <- create_hydro_stats_historical(test_hydro, date_minimum = NA, date_maximum = as.Date("2023-12-31"))
sf_df_currentYr <- create_hydro_stats_singleYr(test_hydro, 2024, WY = "no")

create_hydrograph_separate(st_df_past, sf_df_currentYr, parameter = "flow", output_type = "jpeg", WY = "no"  )
